...
% ----------------------------------------------------------------------- %
function pub_Table2Latex(T, filename, varargin)
    ...
    colAlignment = repmat('l',1,n_col);
    colNames     = T.Properties.VariableNames;
    tabWidth     = '1.0\\textwidth';
    for iArg = 1:2:length(varargin)
        if strcmp(varargin{iArg}, 'colAlignment')
            colAlignment = varargin{iArg+1};
        elseif strcmp(varargin{iArg}, 'colNames')
            colNames = varargin{iArg+1};
        elseif strcmp(varargin{iArg}, 'tabWidth')
            tabWidth = varargin{iArg+1};
        end
    end
    
    % Parameters
    col_spec = colAlignment;
    if ~ischar(colNames)
        colNames = strjoin(colNames, ' & ');
    end
    ...
    fprintf(fileID, ['\\begin{tabularx}{' tabWidth '}[ht]{%s}\n'], col_spec);
    ...
    try
        for row = 1:size(T,1)
            ...
            for col = 1:n_col
                ...
                %if ischar(value) || isstring(value), value = value; end
