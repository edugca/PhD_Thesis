...
for ii=1:nPeriods
    for j=ii:(ii+nFuture)
        ...
        gammaPii = 1;
        for k=1:(j-ii+1)
            gamma = gamma * 1/(1+(1-defPol(ii+k-1)*ddelta(ii+k-1))*phi);
            gammaPii = gammaPii * 1/((1-defPol(ii+k-1)*ddelta(ii+k-1))*phiPii);
        end
        
        if strcmpi(policyTargetRule, 'priceLevel')
            nrGap(j)   = gamma * (rN(j) - (1-defPol(j)*ddelta(j))*iota(j) + defPol(j)*ddelta(j));
        elseif strcmpi(policyTargetRule, 'inflation')
            nrGap(j)   = gammaPii * (rN(j) - (1-defPol(j)*ddelta(j))*iota(j) + defPol(j)*ddelta(j));
        end 
    end
...
%%% rN stochastic; Mon pol tracks the optimal intercept
iSimul       = iSimul + 1;
rN(:)        = rN(:); % use the same sequence of shocks
defPol(:)    = defPol(:); % use the same sequence of shocks
nrBad(:)      = -0.40;
ddelta(:)     = 0.40;
iota(:)      = (rN + defPol.*ddelta) ./ (1-defPol.*ddelta);
...
legend('$\mathcal{D}_0$=0\%', '$\mathcal{D}_0$=3\%', ...
...
ylim([-10,10]);
pub_GraphDrawZeroAxis(graph);
legend('anticipated ($\mathcal{D}_0$=0\%)', 'MIT ($\mathcal{D}_0$=0\%)', ...
    'anticipated ($\mathcal{D}_0$=3\%)', 'MIT ($\mathcal{D}_0$=3\%)', ...
     'Location','southoutside', 'FontSize', fontSizeLegend, 'Interpreter', 'latex', 'NumColumns', 2);
grid 'on';
...
graph(4).LineStyle = '--';
set(gca,'FontSize',fontSize); % Scale fontsize of axes
title("$\mathcal{D}$ goes up by 3 p.p. from t = 1 to 5", 'Interpreter', 'latex', 'FontSize', titleFontSize);
ylabel(ruleLabel, 'Interpreter', 'latex');
xlabel('');
xlim([0,nPlotPeriods]);
ylim([-10,70]);
...
graph(4).LineStyle = '--';
set(gca,'FontSize',fontSize); % Scale fontsize of axes
title("$\delta$ goes up by 20 p.p. from t = 1 to 5", 'Interpreter', 'latex', 'FontSize', titleFontSize);
ylabel(ruleLabel, 'Interpreter', 'latex');
xlabel('', 'Interpreter', 'latex');
xlim([0,nPlotPeriods]);
ylim([-10,50]);
...
graph(3).LineStyle = '-.';
set(gca,'FontSize',fontSize); % Scale fontsize of axes
title("$r^n$ and $\mathcal{D}$ are independent stoch. processes", 'Interpreter', 'latex', 'FontSize', titleFontSize);
ylabel(ruleLabel, 'Interpreter', 'latex');
xlabel('', 'Interpreter', 'latex');
xlim([0,nPlotPeriods]);
ylim([-10,30]);
